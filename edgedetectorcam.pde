  ...
  if (edge){
    ...
    for (int i = 1; i < width-1; i++){
     for (int j = 1; j < height-1; j++){
      ...
      orig[i][j][1] = pixels[j*width+i] & 0xFF;    //blue via bit masking which makes it quicker 
      orig[i][j][2] = pixels[j*width+i] >> 8 & 0xFF;     //green via bit masking which makes it quicker
     }
    }
